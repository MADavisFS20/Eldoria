...
fun main() {
    ...
    if (SaveManager.exists()) {
        ...
        if (resp.startsWith("y", ignoreCase = true)) {
            ...
            if (snap != null) {
                ...
                val session = GameSession(world, snap.player, snap.locationId, snap.homeLocationId, Random(System.nanoTime()))
    ...
    val creationRng = Random(System.nanoTime())
    ...
    val session = GameSession(world, player, startLocation.id, startLocation.id, Random(System.nanoTime()))
...
private fun runGameLoop(session: GameSession) {
    ...
    var lastAutosaveMillis = System.currentTimeMillis()

    while (true) {
        session.advanceTick()
        checkCompanionExpiry(session)
        if (System.currentTimeMillis() - lastAutosaveMillis >= AUTOSAVE_INTERVAL_MILLIS) {
...
private fun hireCompanion(session: GameSession) {
    ...
    session.player = session.player.copy(
        ...
        companion = HiredCompanion(
            ...
            hiredAtMillis = System.currentTimeMillis(),
        ),
    )
...
private fun checkCompanionExpiry(session: GameSession) {
    ...
    if (System.currentTimeMillis() - companion.hiredAtMillis >= HiredCompanion.EMPLOYMENT_DURATION_MILLIS) {
