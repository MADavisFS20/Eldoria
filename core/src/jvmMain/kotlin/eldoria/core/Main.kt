...
import eldoria.core.util.formatOneDecimal
...
fun main() {
    ...
    println(
        "wilderness tiles=${wildernessLocs.size}  empty=${emptyWilderness}  " +
            "(${(100.0 * emptyWilderness / wildernessLocs.size).formatOneDecimal()}% empty)"
    )
    println()
    ...
    println("Tier 1: HP=${tier1.maxHealth} AC=${tier1.armorClass} dmg=${tier1.damage} (avg ${tier1.damage.average().formatOneDecimal()}) worth=${tier1.worth}g magic=${tier1.magicDamage != null}")
    println("Tier 5: HP=${tier5.maxHealth} AC=${tier5.armorClass} dmg=${tier5.damage} (avg ${tier5.damage.average().formatOneDecimal()}) worth=${tier5.worth}g magic=${tier5.magicDamage != null}")
    println()
